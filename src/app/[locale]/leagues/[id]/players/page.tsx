...
export default function PlayersPage() {
  ...
  const { searchTerm, positionFilter, availableFilter, setSearchTerm, setPositionFilter, setAvailableFilter, filteredPlayers, filterStats, resetFilters } = usePlayersFilters({ players });
      ...
      <div className="container mx-auto p-6">
        <Card>
          <CardContent className="p-6">
            <p className="text-center text-muted-foreground">{t("errors.leagueNotFound")}</p>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      {/* Stats */}
      <PlayersStats players={players} leagueStatus={league.status} />

      {/* Actions */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <PlayersActions canModifyPlayers={canModifyPlayers} onImportPlayers={openImportForm} onRefreshPlayers={fetchPlayers} loading={loading} />
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{t("players.playersListDesc")}</CardDescription>
        </CardHeader>
        <CardContent>
          <PlayersTable
      ...
      {showImportForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <PlayersImportForm uploading={uploading} onImport={handleImport} onClose={closeImportForm} />
        </div>
      )}
    </div>
  );
