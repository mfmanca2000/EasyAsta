    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">{t("players.totalPlayers")}</CardTitle>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">{t("players.available")}</CardTitle>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle className="text-sm font-medium">{t("players.assigned")}</CardTitle>
      ...
      <Card className="md:col-span-2 lg:col-span-4">
        <CardHeader>
          <CardTitle className="text-sm font-medium">{t("players.byPosition")}</CardTitle>
          <CardDescription>{t("players.positionBreakdown")}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {(["P", "D", "C", "A"] as const).map((position) => {
              const positionData = calculatedStats.byPosition[position];
              const percentage = positionData.total > 0 ? ((positionData.assigned / positionData.total) * 100).toFixed(1) : "0";

              return (
                <div key={position} className="text-center space-y-2">
                  <div className="font-medium text-lg">{t(`players.positions.${position}`)}</div>
                  <div className="space-y-1">
                    <div className="text-2xl font-bold">{positionData.total}</div>
