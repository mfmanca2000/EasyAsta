...
export async function calculateBotSelection(userId: string, leagueId: string, roundId: string, intelligence: "LOW" | "MEDIUM" | "HIGH" = "MEDIUM"): Promise<BotSelectionResult | null> {
  try {
...
function selectBalancedPlayer(players: Player[], team: TeamWithPlayers, needed: PositionNeeds, position: keyof PositionNeeds): BotSelectionResult {
...
async function selectOptimalPlayer(players: Player[], team: TeamWithPlayers, needed: PositionNeeds, position: keyof PositionNeeds, leagueId: string): Promise<BotSelectionResult> {
  try {
...
export async function removeBotUsers(leagueId: string): Promise<void> {
  ...
  const botUserIds = botTeams.filter((team) => team.user.isBot).map((team) => team.user.id);
